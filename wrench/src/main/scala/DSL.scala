...
def (test: TestCase) shouldCompile(implicit ctx: TestContext): Unit = ctx.transaction(try {
...
def (test: TestCase) checkCompile(implicit ctx: TestContext): Unit = ctx.transaction(try {
...
def (test: TestCase) shouldRun(implicit ctx: TestContext): Unit = ctx.transaction(try {
